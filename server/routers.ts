...
import { generatedPlanSchema, planningChatInputSchema, planConversation } from "./planning";
import { createConversation, deactivateMemory, getActionableSessions, getConversation, getConversationSessions, getFocusSession, listConversations, listMemories, resolveFocusSession, saveConversation, saveMemories, startFocusSession, type StoredMessage } from "./plannerStore";
...
export const appRouter = router({
  ...
  planning: router({
    chat: publicProcedure.input(planningChatInputSchema).mutation(async ({ input }) => {
      ...
      const updatedHistory = [...history, { role: "user" as const, content: input.message }];
      const reply = await planConversation(updatedHistory, memories.map((item) => ({ kind: item.kind, content: item.content })));
      const assistantContent = reply.status === "needs_context" && reply.missingDetail ? `${reply.assistantMessage}\n\n**سؤالي التالي:** ${reply.missingDetail}` : reply.assistantMessage;
