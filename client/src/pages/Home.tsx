...
import { Archive, BotMessageSquare, BrainCircuit, Clock3, History, Instagram, MessageCircleMore, Plus, RotateCcw, Send, Sparkles, X } from "lucide-react";
...
export default function Home() {
  ...
  const currentSession = allSessions.find((item) => item.status === "running") ?? actionable.find((item) => item.status === "running") ?? null;
  ...
  const updatePlanMutation = trpc.planning.updatePlan.useMutation({ onSuccess: () => utils.planning.list.invalidate({ workspaceId }), onError: (error) => toast.error(error.message) });
  const startMutation = trpc.focus.start.useMutation({ onSuccess: () => { if (currentConversationId) utils.focus.listForConversation.invalidate({ workspaceId, conversationId: currentConversationId }); utils.focus.actionable.invalidate({ workspaceId }); setTab("session"); toast.success("بدأت جلسة التركيز وحُفظ وقت نهايتها."); }, onError: (error) => toast.error(error.message) });
  const resolveMutation = trpc.focus.resolve.useMutation({ onSuccess: (result) => { const conversationId = result.conversationId ?? currentConversationId; if (conversationId) { utils.focus.listForConversation.invalidate({ workspaceId, conversationId }); utils.planning.get.invalidate({ workspaceId, conversationId }); } utils.focus.actionable.invalidate({ workspaceId }); utils.planning.list.invalidate({ workspaceId }); }, onError: (error) => toast.error(error.message) });
  const repairMutation = trpc.planning.repair.useMutation({ onSuccess: (reply) => { setRepairSession(null); setObstacle(""); setCurrentConversationId(reply.conversationId); setMessages(reply.messages); if (reply.plan) setPlan(reply.plan); utils.focus.listForConversation.invalidate({ workspaceId, conversationId: reply.conversationId }); utils.planning.get.invalidate({ workspaceId, conversationId: reply.conversationId }); utils.focus.actionable.invalidate({ workspaceId }); utils.planning.list.invalidate({ workspaceId }); utils.planning.memories.invalidate({ workspaceId }); setTab("chat"); toast.success(reply.plan ? "فهم فكّك العائق وعدّل الخطة." : "فهم فكّك العائق ويسأل عن التفصيل الأخير."); }, onError: (error) => toast.error(error.message) });
  const forgetMutation = trpc.planning.forgetMemory.useMutation({ onSuccess: () => utils.planning.memories.invalidate({ workspaceId }) });

  const sendMessage = (content: string) => chatMutation.mutate({ workspaceId, conversationId: currentConversationId ?? undefined, message: content });
  ...
  const startStep = (step: ConversationPlanData["steps"][number]) => { if (!currentConversationId) return toast.error("أرسل الخطة أولًا حتى نستطيع حفظ الجلسة."); startMutation.mutate({ workspaceId, conversationId: currentConversationId, stepOrder: step.order, stepTitle: step.action, durationSeconds: durationFor(step) }); };
  ...
  const completeFromTimer = () => { if (!awaiting) return; resolveMutation.mutate({ workspaceId, sessionId: awaiting.id, outcome: "completed" }); toast.success("تم وضع علامة إنجاز على الخطوة."); };
  ...
  const sessionState: FocusState[] = [...allSessions, ...actionable].map((item) => ({ stepOrder: item.stepOrder, status: item.status, endsAt: item.endsAt, durationSeconds: item.durationSeconds }));

  return <div className="chat-app" dir="rtl"><main className="chat-shell">
    <header className="chat-header"><div className="chat-brand"><span className="chat-brand-mark"><img src="/manus-storage/fakkik-symbol-logo_819d919f.png" alt="رمز فكّك" /></span><div><strong>فكّك</strong><span>ذاكرة تخطيط مستمرة</span></div></div><button className="new-conversation" onClick={newConversation}><RotateCcw size={16} /> محادثة جديدة</button></header>
    <nav className="app-tabs" aria-label="أقسام فكّك"><button className={tab === "chat" ? "active" : ""} onClick={() => setTab("chat")}><BotMessageSquare size={16} /> المحادثة</button><button className={tab === "session" ? "active" : ""} onClick={() => setTab("session")}><Clock3 size={16} /> جلسة الآن{currentSession && <i />}</button><button className={tab === "history" ? "active" : ""} onClick={() => setTab("history")}><History size={16} /> المحفوظات</button></nav>
    {tab === "chat" && <>
      ...
      {plan ? <ConversationPlan plan={plan} sessions={sessionState} onStart={startStep} onToggle={toggleStep} /> : <section className="plan-waiting"><MessageCircleMore size={20} /><div><strong>ابدأ بحوار واحد واضح.</strong><span>يطلب فكّك التفاصيل المؤثرة فقط، ثم يحفظ الخطة لتعود إليها في أي وقت.</span></div></section>}
    </>}
    {tab === "session" && <section className="section-screen"><div className="section-title"><span>جلسة الآن</span><h1>التركيز الجاري</h1><p>العداد محفوظ بوقت نهاية حقيقي. عند انتهائه يظل سؤال الإكمال ظاهرًا حتى تجيب.</p></div><FocusSessionCard session={currentSession} />{awaiting && <button className="review-now" onClick={() => setTab("chat")}>لدي جلسة انتهت · أجب الآن</button>}</section>}
    ...
    <footer className="chat-footer"><Plus size={13} /> كل خطة تبدأ بمحادثة لا بتخمين <span>تطوير يازِين · <a href="https://instagram.com/pro_hg_i" target="_blank" rel="noreferrer"><Instagram size={12} />@pro_hg_i</a> · صنع من قبل نور</span></footer>
    ...
    {repairSession && <div className="repair-layer" role="dialog" aria-modal="true"><section className="repair-sheet"><button onClick={() => setRepairSession(null)} aria-label="إغلاق"><X size={18} /></button><span>علاج العائق</span><h2>ما الذي أوقفك في هذه الخطوة؟</h2><p>{repairSession.stepTitle}</p><textarea value={obstacle} onChange={(event) => setObstacle(event.target.value)} placeholder="مثال: الوقت أقل من المتوقع، أو لم أفهم الجزء المطلوب، أو كانت المهمة مرهقة…" /><button className="repair-send" onClick={sendRepair} disabled={repairMutation.isPending || obstacle.trim().length < 3}><Send size={15} /> دع فكّك يعدّل الخطة</button></section></div>}
  </main></div>;
}
