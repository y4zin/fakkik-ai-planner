...
import { Archive, BotMessageSquare, BrainCircuit, Clock3, History, Instagram, ListChecks, MessageCircleMore, Plus, RotateCcw, Sparkles, X } from "lucide-react";
...
export default function Home() {
  ...
  const startMutation = trpc.focus.start.useMutation({ onSuccess: () => { if (currentConversationId) utils.focus.listForConversation.invalidate({ workspaceId, conversationId: currentConversationId }); utils.focus.actionable.invalidate({ workspaceId }); utils.focus.listWorkspace.invalidate({ workspaceId }); setTab("session"); toast.success("بدأت جلسة التركيز وحُفظ وقت نهايتها."); }, onError: (error) => toast.error(error.message) });
  const resolveMutation = trpc.focus.resolve.useMutation({ onSuccess: (result) => { const conversationId = result.conversationId ?? currentConversationId; if (conversationId) { utils.focus.listForConversation.invalidate({ workspaceId, conversationId }); utils.planning.get.invalidate({ workspaceId, conversationId }); } utils.focus.actionable.invalidate({ workspaceId }); utils.focus.listWorkspace.invalidate({ workspaceId }); utils.planning.list.invalidate({ workspaceId }); }, onError: (error) => toast.error(error.message) });
  ...
  return <div className="chat-app" dir="rtl"><main className="chat-shell">
    ...
    {tab === "progress" && <ExecutionBoard plan={plan} conversationId={currentConversationId} sessions={workspaceSessions} onStart={startStep} onReview={() => awaiting ? setTab("chat") : setTab("session")} />}
